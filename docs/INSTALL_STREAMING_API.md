...
**Auto mode (experimental, off by default).** `POST /{id}/install` accepts `auto_mode` (`null` uses `install.auto_mode` from config.yml). With it on, the worker OCRs the installer screen (Tesseract, languages from `INSTALL_AUTO_OCR_LANGS`) and presses known buttons: Next, Agree (only on pages that mention a license/EULA keyword), Install, Finish. Each button is clicked first; if the page does not change, its Alt+letter mnemonic is tried. The button list lives in `backend/handler/install/auto_mode/buttons.yml` and can be extended from config.yml (`install.auto_mode_extra_buttons`, same `{category, labels, mnemonic, toggle}` shape). `PATCH /{id}/install/auto-mode` `{enabled}` flips it on a running session. The session carries `auto_mode`, `auto_status` (`running` | `needs_manual`) and `auto_detail` (last action). `needs_manual` means no known button, page change or file growth for `INSTALL_AUTO_STUCK_SECONDS`: clients should tell the user to continue by hand through the VNC page (the web UI shows a warning, the CLI prints it). Both fields are `null` while auto mode is off or after the installer exits.
